...
for(smp in unique(bleed_through_slope_df$sample)){
        ...
        df = bleed_through_slope_df%>%filter(sample==smp) %>% distinct()
        df_lbl = bleed_through_labels%>%filter(sample==smp) %>% distinct()
...
pdf(file.path(opt$plots_basedir, "barcode_correlations.pdf"), width = 8, height = 8)
# Start the plot 
...
#     distinct() %>%
#     write.table(
#         file = file.path(opt$activity_basedir, "activity.txt"),
#         row.names = FALSE, quote = F, sep = "\t"
#     )
# corrected_activity_df %>%
#     left_join(design_df %>% select(replicate, sample), by = c("cDNA_sample" = "replicate")) %>%
...
#     distinct() %>%
#     write.table(
#         file = file.path(opt$activity_basedir, "corrected_activity.txt"),
#         row.names = FALSE, quote = F, sep = "\t"
#     )
