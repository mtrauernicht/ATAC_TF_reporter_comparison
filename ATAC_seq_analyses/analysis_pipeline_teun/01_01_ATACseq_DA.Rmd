...
setwd("~/projects/ATAC_TF_reporter_comparison/ATAC_seq_analyses")

exp <- readRDS("~/projects/ATAC_TF_reporter_comparison/ATAC_seq_analyses/rds/experiment_7784_20250206.rds")
exp$replicate[exp$celltype == "mES" & exp$treatment == "2i_LIF" & exp$replicate == "R3" & exp$run == "8011"] <- "R4"
```
...
df <- df %>%
  filter(!(treatment == "2i_LIF" & replicate == "R3"))

write_csv(df, "/DATA/shared/projects/TF_REPORTERS/ATAC_TF_reporter_comparison/ATAC_seq_analyses/bigwig/size_factors_deseq_mt20250423.csv")
...
diff_acc <- results(exp1, contrast = c("treatment", "LIF", "2i_LIF"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("2i_LIF" %->% "LIF"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
...
diff_acc <- results(exp1, contrast = c("treatment", "CH", "2i_LIF"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("2i_LIF" %->% "CH"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
...
diff_acc <- results(exp1, contrast = c("treatment", "PD", "2i_LIF"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("2i_LIF" %->% "PD"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
